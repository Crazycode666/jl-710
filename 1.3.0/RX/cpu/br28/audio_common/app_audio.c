...
void app_audio_set_volume(u8 state, s8 volume, u8 fade)
{
    ...
    if (SYS_VOL_TYPE == VOL_TYPE_DIGGROUP && SYS_DIGVOL_GROUP_EN) {
        ...
        audio_dig_vol_group_vol_set_by_head(sys_digvol_group, digvol_type, AUDIO_DIG_VOL_ALL_CH, volume);
        return;
    }
...
void app_audio_set_volume_each_channel(u8 state, s8 volume_l, s8 volume_r, u8 fade)
{
    ...
    if (SYS_VOL_TYPE == VOL_TYPE_DIGGROUP && SYS_DIGVOL_GROUP_EN) {
        ...
        audio_dig_vol_group_vol_set_by_head(sys_digvol_group, digvol_type, AUDIO_DIG_VOL_CH(0), volume_l);
        audio_dig_vol_group_vol_set_by_head(sys_digvol_group, digvol_type, AUDIO_DIG_VOL_CH(1), volume_r);
        return;
    }
#endif //vol_type_diggroup
