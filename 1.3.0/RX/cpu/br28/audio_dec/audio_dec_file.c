...
static struct file_audio_wait_capture *file_wait_capture;
...
{
    if (file_wait_capture && file_wait_capture->timer) {
    ...
    if (event == AUDIO_RES_GET) {
        ...
        if (file_dec->file_dec.status) {
            if (file_dec->file_dec.status == FILE_DEC_STATUS_PLAY || \
                ...
                extern int music_player_fade_out(void);
