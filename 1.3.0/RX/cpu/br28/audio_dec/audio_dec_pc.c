...
*/
/*----------------------------------------------------------------------------*/
extern dvol_handle *chat_dev_dvol;
u32 user_mvol = 8;
static int usb_device_event_handler(u8 event, int value)
{
    ...
    case USB_AUDIO_SET_PLAY_VOL:
        ...
        u32 mvol_l, mvol_r = 0;
        mvol_l = uac_vol_switch(value & 0xffff);  //left ch volume
        mvol_r = uac_vol_switch((value >> 16) & 0xffff);
        user_mvol = mvol_l;
        if((mvol_l == 0 || mvol_r == 0) && (value != 0))
        {
            user_mvol = 1;
        }
        audio_digital_vol_set(chat_dev_dvol,user_mvol);
        //u32 vol_l, vol_r = 0;
