...
#if (SOUNDCARD_ENABLE || MULTI_AUDIO_UPLOAD_TO_UAC_ENABLE)
    usb_mix_fifo = mix_fifo_init(1/*MIC_CHANNEL*/, 48000);
#endif
...
#if (RECORDER_MIX_EN)
        // 录音
        audio_mixer_set_sample_rate(&recorder_mixer, MIXER_SR_SPEC, sr);
#endif//RECORDER_MIX_EN
    }
...
#if AUDIO_VOCAL_REMOVE_EN
    mix_vocal_remove_hdl = vocal_remove_open(ch_num, sr, 100, 200, 13000);
