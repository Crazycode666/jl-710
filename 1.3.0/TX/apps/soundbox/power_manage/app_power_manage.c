...
void vbat_led_states()
{
    static u16 power = 0;
    power = get_self_battery_level() * 10 + 10; //显示10%~100%
    if(power >= 40)
    {
        rgb_led_on(TCFG_PWMLED_PIN1);
        rgb_led_off(TCFG_PWMLED_PIN2);
        rgb_led_off(TCFG_PWMLED_PIN0);
        return;
    }
    else if(power > 20)
    {
        rgb_led_on(TCFG_PWMLED_PIN2);
        rgb_led_off(TCFG_PWMLED_PIN1);
        rgb_led_off(TCFG_PWMLED_PIN0);
        return;
    }
    else if(power > 10)
...
void charge_vbat_led_states_add()
{
    if(!charge_vbat_flag)
    {
        charge_vbat_flag = sys_timer_add(NULL,charge_vbat_led_states,1200);
    }
}
...
static u16 vbat_flag = 0;
void vbat_led_states_add()
{
    if(!vbat_flag)
    {
        vbat_flag = sys_timeout_add(NULL,vbat_led_states,5000);
    }
}

void vbat_led_states_del()
{
    if(vbat_flag)
    {
        rgb_led_off(TCFG_PWMLED_PIN0);
