...
void rgb_led_on(u32 gpio_port)
{
            //常亮
    gpio_set_direction(gpio_port, 0);
    gpio_set_output_value(gpio_port, 0);
}

void rgb_led_off(u32 gpio_port)
{
            //常亮
    gpio_set_direction(gpio_port, 0);
...
void echo_led_on(u32 gpio_port)
{
    gpio_set_direction(gpio_port, 0);
    gpio_set_output_value(gpio_port, 1);
}

void echo_led_off(u32 gpio_port)
{
    gpio_set_direction(gpio_port, 0);
    gpio_set_output_value(gpio_port, 0);
}

void gpio_led_high_block(u32 led_gpio)
{
    gpio_set_pull_up(led_gpio, 0);
    gpio_direction_input(led_gpio);
    gpio_set_pull_down(led_gpio, 0);
    gpio_set_output_value(led_gpio, 0);
    gpio_set_die(led_gpio, 0);
}
...
void led_flash_add()
{
    if(!led_time)
    {
        led_time = sys_timer_add(NULL,led_flash,1000*10);
    }
}
...
{
    echo_led_on(TCFG_PWMLED_PIN1);
    os_time_dly(20);
    echo_led_off(TCFG_PWMLED_PIN1);
    echo_led_on(TCFG_PWMLED_PIN2);
    os_time_dly(20);
    echo_led_off(TCFG_PWMLED_PIN2);
...
void led_water_flash_add()
{
    if(!led_water_time)
    {
        led_water_time = sys_timer_add(NULL,led_water_flash,800);
    }
}
