        ...
        struct audio_data_frame *out)
{
    ...
    struct audio_frame frame = {
        ...
        .nch = a2dp_dec->nch,
        .sample_rate = decoder->fmt.sample_rate,
        .delay = a2dp_media_get_remain_play_time(0),
        .len = in->data_len,
        .data = in->data,
        .offset = in->offset,
        .timestamp = a2dp_dec->capture_time ? a2dp_dec->capture_time(a2dp_dec->clock, 0) : 0,
    };

    wlen = a2dp_dec->write_frame(a2dp_dec->capture_path, &frame);
    /*printf("write : 0x%x, %d, %d\n", (u32)a2dp_dec->capture_path, in->data_len, wlen);*/
...
static int a2dp_pcm_capture_start(void)
{
    ...
    ch_num = dec->nch;
    ch_type = dec->nch == 2 ? AUDIO_CH_LR :	AUDIO_CH_DIFF;
...
static int a2dp_dec_start(void)
{
...
#if (SOUNDCARD_ENABLE)
    bt_mix_fifo_ch = mix_fifo_ch_open(usb_mix_fifo, audio_output_channel_num());
#endif
    ...
    vocal_remove_samplerate_set(fmt->sample_rate);
#endif
