...
#define LIVE_JLA_STREAM_PLAY_LATENCY         (15 * 1000L)
...
#else
#define LIVE_JLA_STREAM_PLAY_LATENCY(dec_latency, frame_len)  (LIVE_STREAM_PLAY_LATENCY(dec_latency) + ((frame_len / 10) * 1000L))
...
static int live_player_music_effect_stream_entry_add(struct live_stream_player_context *ctx, struct audio_stream_entry **entries, int max)
{
    ...
    if (ctx->music_effect.bass_sel) {	//接收端
        ...
        if (ctx->music_effect.eq) {
            ...
            if (ctx->music_effect.dy_eq && ctx->dy_eq->dy_eq) {
...
static int live_stream_player_start(struct live_stream_player_context *ctx)
{
...
#if (RECORDER_MIX_EN)
    audio_mixer_ch_open_head(&ctx->rec_mix_ch, &recorder_mixer);
#if (!SOUNDCARD_ENABLE)
    audio_mixer_ch_set_no_wait(&ctx->rec_mix_ch, 1, 10); // 超时自动丢数
#endif
#endif/*RECORDER_MIX_EN*/
...
#endif
    // 数据流串联
    struct audio_stream_entry *entries[20] = {NULL};
    u8 entry_cnt = 0;
    ...
    void *dvol_entry = sys_digvol_group_ch_open("music_transmit", -1, NULL);
...
#if (RECORDER_MIX_EN)
    audio_stream_add_entry(entries[entry_cnt - 2], &ctx->rec_mix_ch.entry);
#endif/*RECORDER_MIX_EN*/
...
#if (RECORDER_MIX_EN)
    audio_mixer_ch_close(&ctx->rec_mix_ch);
#endif/*RECORDER_MIX_EN*/
...
int live_stream_player_capture(void *priv, struct audio_path *path)
{
    ...
    printf("live stream player capture open.\n");
