    ...
    if ((JL_SYSTEM->CHIP_VER >= 0xA2) && (JL_SYSTEM->CHIP_VER < 0xAC)) { //C版以后才做DAC TRIM
        ...
        if (len != sizeof(dac_trim) || common->audio_trim_flag) {
            ...
            if (config_audio_dac_output_mode == DAC_MODE_SINGLE) {
                ...
                if (dac_hdl.pd->power_mode > 4) {
                    dac_trim.left = (config_audio_dac_output_mode == DAC_MODE_DIFF) ? (-1250) : (-2500);
                    dac_trim.right = (config_audio_dac_output_mode == DAC_MODE_DIFF) ? (-1250) : (-2500);
                } else {
                    ...
                    int trim_limit = (config_audio_dac_output_mode == DAC_MODE_DIFF) ? (375) : (750);
