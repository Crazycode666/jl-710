...
void pc_spk_data_isr_cb(void *buf, u32 len)
{
    ...
    if (len) {
        ...
        if ((cache_len >= hdl->frame_len * SPK_PUSH_FRAME_NUM) && !pc_cached_flag) {
            pc_cached_flag = 1;
            return;
        }
        frame = source_plug_get_output_frame(source_node, hdl->frame_len);
        if (!frame) {
            pc_spk_isr.pc_spk_in_isr = 0;
            return;
        }
...
#else
        hdl->lost_cnt = 0;
        if (cache_len < hdl->frame_len * SPK_PUSH_FRAME_NUM) {
            return;
        }
...
#else
        frame = source_plug_get_output_frame(source_node, hdl->frame_len * SPK_PUSH_FRAME_NUM);
        if (!frame) {
            pc_spk_isr.pc_spk_in_isr = 0;
            return;
        }
