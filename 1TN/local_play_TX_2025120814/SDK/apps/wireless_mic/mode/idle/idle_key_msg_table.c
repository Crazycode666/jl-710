...
int idle_key_power_msg_remap(int *msg)
{
    ...
    case KEY_ACTION_HOLD:
        //长按切换ANC模式
        app_msg = APP_MSG_KEY_POWER_OFF_HOLD;
        break;
    default:
        break;
    }
    printf("idle_key_msg_remap, key_action: %d, app_msg: %d\n", key_action, app_msg);
    return app_msg;
}


int idle_key_slider_msg_remap(int *msg)
{
    int app_msg = APP_MSG_NULL;
    u8 key_action  = APP_MSG_KEY_ACTION(msg[0]);
...
const struct key_remap_table idle_mode_key_table[] = {
    ...
    { .key_value = KEY_POWER,   .remap_func = idle_key_power_msg_remap },
    { .key_value = KEY_SLIDER,  .remap_func = idle_key_slider_msg_remap },
#endif
