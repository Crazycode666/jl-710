...
static bool broadcast_rx_badv_data_callback(const void *const buf, size_t len, uint8_t big_hdl);
#endif

static void broadcast_custom_data_receive_callback(const void *const buf, size_t length, void *priv);
/**************************************************************************************************
...
const big_callback_t big_tx_cb = {
    .receive_packet_cb      = broadcast_custom_data_receive_callback,
...
const big_callback_t big_rx_cb = {
    ...
    .receive_badv_data_cb   = broadcast_rx_badv_data_callback,
...
extern const char config_product_scan_rssi;
 ...
 */
/* ----------------------------------------------------------------------------*/
int broadcast_transmitter_connect_deal(void *priv)
{
    u8 i;
    ...
    params.service_type = LEA_SERVICE_WL_MIC;
...
static void broadcast_tx_event_callback(const BIG_EVENT event, void *priv)
{
    ...
    case BIG_EVENT_TRANSMITTER_DISCONNECT:
        ...
        big_event_to_user(event, priv, 0);
        break;
...
static void broadcast_printf_string_buf(const uint8_t *buf, size_t len)
{
    while (len--) {
        if (*buf == 0) {
            break;
 ...
 * @param big_hdl：big句柄
 *
 * @return TRUE-允许建立连接，FALSE-不允许建立连接
 */
/* ----------------------------------------------------------------------------*/
static bool broadcast_rx_badv_data_callback(const void *const buf, size_t len, uint8_t big_hdl)
{
    ...
    return TRUE;
#endif
    le_adv_report_evt_t *report_pt = (le_adv_report_evt_t *)buf;
    int8_t rssi = report_pt->Data[report_pt->Data_Length];
    uint8_t length, ad_type;
    uint8_t data_length = report_pt->Data_Length;
    uint8_t *adv_data_pt = report_pt->Data;
    uint8_t find_remoter = 0;
    ...
    for (int i = 0; i < data_length;) {
        ...
        case HCI_EIR_DATATYPE_SHORTENED_LOCAL_NAME:
            ...
            /* log_info("target name: %s", target_name); */
            /* log_info("=remoter_name: "); */
            /* broadcast_printf_string_buf(adv_data_pt, name_len); */
    ...
    //判断是否符合rssi限制
    if (config_product_scan_rssi && (rssi > config_product_scan_rssi)) {
        find_remoter = TRUE;
    }
...
int broadcast_send_custom_data(u16 aux_hdl, void *data, size_t length)
{
    ...
    if (broadcast_role == BROADCAST_ROLE_TRANSMITTER) {
        err = wireless_trans_transmit("big_tx", data, length, &param);
    } else if (broadcast_role == BROADCAST_ROLE_RECEIVER) {
        err = wireless_trans_transmit("big_rx", data, length, &param);
    } else {
        printf("broadcast error role ,cant send custom data");
        return err;
    }
    if (err != 0) {
