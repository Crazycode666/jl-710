...
const int config_audio_dac_underrun_detect_enable = 0; //使能1ms欠载检测,无线mic低延时关掉
#else
const int config_audio_dac_underrun_detect_enable = 0; //使能1ms欠载检测,
#endif
//<DAC FIFO Mixer>
/*
...
#ifdef TCFG_JLA_V2_USED_FRAME_LEN_MASK
const unsigned short JLA_V2_FRAMELEN_MASK = BIT(15) | TCFG_JLA_V2_USED_FRAME_LEN_MASK;
#else
...
const int JLA_V2_PLC_FADE_OUT_POINTS = 120 * 10;    //plc维持指定点数后,淡出的速度,音量从满幅到0需要的点数.
const int JLA_V2_PLC_FADE_IN_POINTS = 120 * 10;     //plc后收到正确包淡入,淡入的速度,音量从0到满幅需要的点数.

//jla_v2编解码是否带2个byte的头信息(表示有效编码数据的长度)，注意这2个byte 的头信息没有算到编解码的码率里面
const int JLA_V2_CODEC_WITH_FRAME_HEADER = 0;

const char config_lib_jla_v2_codec_debug_level = 0;
...
const u32 LLNS_DNS_SUPPORT_SAMPLE_RATE = TCFG_AUDIO_ADC_SAMPLE_RATE;
const u16 LLNS_DNS_PROCESS_FRAME_SIZE = (LLNS_DNS_SUPPORT_SAMPLE_RATE == 32000) ? 480 : 720; //降噪一次输出数据长度(点数)，请在原厂指导下更改
