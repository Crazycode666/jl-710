...
#if defined(TCFG_HI_RES_AUDIO_ENEBALE) || TCFG_VIRTUAL_SURROUND_EFF_MODULE_NODE_ENABLE
//调整解码器输出帧长
...
#if (defined(WIRELESS_MIC_PRODUCT_MODE) && (WIRELESS_MIC_PRODUCT_MODE == ADAPTER_2T1R_MODE))
...
#if ((defined(WIRELESS_MIC_PRODUCT_MODE) && (WIRELESS_MIC_PRODUCT_MODE == ADAPTER_2T1R_MODE)) && LEA_BIG_FIX_ROLE == LEA_ROLE_AS_TX)
static int get_adapter_mic_local_monitor_status()
{
    int state = 1;//获取本地监听打开状态
    return state;
}
#endif
...
#if ((defined(WIRELESS_MIC_PRODUCT_MODE) && (WIRELESS_MIC_PRODUCT_MODE == ADAPTER_2T1R_MODE)) && LEA_BIG_FIX_ROLE == LEA_ROLE_AS_TX)
    //领夹mic tx 本地监听 开关判断
    if (!strcmp(arg, "SW_Monitor")) {
        return (int)get_adapter_mic_local_monitor_status;
    }
#endif
