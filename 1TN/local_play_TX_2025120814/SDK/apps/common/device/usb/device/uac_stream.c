...
static void usb_sof_spk_write_callback(struct usb_device_t *usb_device, u32 ep, u32 frame)
{
    ...
    u16 len;
    u8 index = uac_speaker->buf_index ^ 1;

    if (speaker_stream_is_open && uac_speaker->spk_double_buf) {
        memcpy(&len, &uac_speaker->spk_double_buf[index][0], 2);
        pc_spk_data_isr_cb(&uac_speaker->spk_double_buf[index][2], len);
        memset(&uac_speaker->spk_double_buf[index][0], 0, 2);
    }
#endif
}
...
void uac_speaker_stream_write(const u8 *obuf, u32 len)
{
    ...
    if (speaker_stream_is_open) {
        if (uac_speaker->stream_state == 0) {
            ...
            if (!uac_speaker->spk_double_buf) {
                int row = 2;
                int col = (512 * (uac_speaker->bitwidth == 24 ? 2 : 1) * uac_speaker->channel);
                uac_speaker->spk_double_buf = (u8 **)zalloc(row * sizeof(u8 *));
                ASSERT(uac_speaker->spk_double_buf);
                for (int i = 0; i < row; i++) {
                    uac_speaker->spk_double_buf[i] = zalloc(col);
                    ASSERT(uac_speaker->spk_double_buf[i]);
                }
            }
            uac_speaker->buf_index = 0;
    ...
    if (uac_speaker->spk_double_buf) {
        memcpy(&uac_speaker->spk_double_buf[uac_speaker->buf_index][0], &len, 2);
        memcpy(&uac_speaker->spk_double_buf[uac_speaker->buf_index][2], obuf, len);
    }
    uac_speaker->buf_index ^= 1;
#endif
}
...
void uac_speaker_stream_open(u32 samplerate, u32 ch, u32 bitwidth)
{
    ...
    uac_speaker->samplerate = samplerate;
    uac_speaker->bitwidth = bitwidth;
    uac_speaker->spk_double_buf = NULL;

    speaker_stream_is_open = 1;

#if TCFG_USB_SLAVE_AUDIO_SPK_ENABLE
    printf(">> Case : USB_AUDIO_PLAY_OPEN\n");
    pc_spk_set_fmt(ch, bitwidth, samplerate);
...
void uac_speaker_stream_close_delay(void *priv)
{
    ...
    if (uac_speaker) {
        ...
        if (uac_speaker->stream_state) {
#if TCFG_USB_SLAVE_AUDIO_SPK_ENABLE
            printf(">> Case : USB_AUDIO_PLAY_CLOSE\n");
            if (release) {
                pc_spk_player_close();
            } else {
        ...
        usb_g_set_sof_hander(0, 0, NULL);
        if (uac_speaker->spk_double_buf) {
            int row = 2;
            for (int i = 0; i < row; i++) {
                if (uac_speaker->spk_double_buf[i]) {
                    free(uac_speaker->spk_double_buf[i]);
                    uac_speaker->spk_double_buf[i] = NULL;
                }
            }
            free(uac_speaker->spk_double_buf);
            uac_speaker->spk_double_buf = NULL;
        }
