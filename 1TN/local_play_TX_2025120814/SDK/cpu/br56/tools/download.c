...
$compress_tool -dict text.bin -input data_code_z.bin 0 data.bin 0 data_code.bin 0 init.bin 0 aec.bin 0 aac.bin 0 $bank_files -o compress.bin
#else
$compress_tool -dict text.bin -input data_code_z.bin 0 init.bin 0 aec.bin 0 aac.bin 0 $bank_files -o compress.bin
#endif

#ifdef CONFIG_LZ4_DATA_CODE_ENABLE
cat text.bin compress.bin i0_ram_data_code.bin > app.bin
#else
cat text.bin data.bin data_code.bin compress.bin i0_ram_data_code.bin > app.bin
#endif
...
files="app.bin  br56loader.uart br56loader.bin uboot.boot ota*.bin isd_config.ini"
...
#ifdef CONFIG_LZ4_DATA_CODE_ENABLE
%compress_tool% -dict text.bin -input data_code_z.bin 0 data.bin 0 data_code.bin 0 init.bin 0 aec.bin 0 aac.bin 0 !bankfiles! -o compress.bin
#else
...
#ifdef CONFIG_LZ4_DATA_CODE_ENABLE
copy /b text.bin + compress.bin + i0_ram_data_code.bin app.bin
#else
copy /b text.bin + data.bin + data_code.bin + compress.bin + i0_ram_data_code.bin app.bin
#endif

del !bankfiles! data_code_z.bin text.bin data.bin compress.bin i0_ram_data_code.bin
...
#elif defined(CONFIG_WIRELESS_MIC_CASE_ENABLE)
call download/wireless_mic/download.bat
