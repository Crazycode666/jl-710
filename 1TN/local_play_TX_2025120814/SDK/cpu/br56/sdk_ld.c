...
FREE_IACHE0_WAY = TCFG_FREE_ICACHE0_WAY_NUM; //max is 3  // 如果同时了开启低功耗会有问题
ICACHE0_RAM_SIZE = TCFG_FREE_ICACHE0_WAY_NUM*8K;

MEMORY
{
	code0(rx)    	  : ORIGIN = CODE_BEG,  LENGTH = CONFIG_FLASH_SIZE
	ram0(rwx)         : ORIGIN = RAM0_BEG,  LENGTH = RAM0_SIZE
    icache0_ram(rw)    : ORIGIN =  0x3C0000+((4-FREE_IACHE0_WAY)*8K), LENGTH = ICACHE0_RAM_SIZE
}
...
#include "btctrler/port/br56/btctler_lib.ld"
...
data_code_z_begin = text_begin + text_size;
#else
...
#ifndef CONFIG_CODE_MOVABLE_ENABLE
icache0_ram_data_code_begin = aac_begin + aac_size;
#else
icache0_ram_data_code_begin = data_code_z_begin + data_code_z_size;
#endif
