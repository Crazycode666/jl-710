...
void echo_led_on(u32 port,u32 pin,u32 gpio_port)
{
            //常亮
    //gpio_set_direction(gpio_port,0);
    //gpio_set_output_value(gpio_port,1);
    //gpio_set_mode(port,pin,PORT_OUTPUT_HIGH);
    //gpio_write(gpio_port,1);
    gpio_set_mode(port,pin,PORT_OUTPUT_HIGH);
    gpio_write(gpio_port,1);
}

void echo_led_off(u32 port,u32 pin,u32 gpio_port)
{
            //常灭
    gpio_set_mode(port,pin,PORT_OUTPUT_LOW);
    gpio_write(gpio_port,0);
}

void gpio_led_high_block(u32 port,u32 pin,u32 gpio_port)
{
    gpio_set_mode(port,pin,PORT_INPUT_FLOATING);
    gpio_write(gpio_port,0);
}

static u16 led_time = 0;

extern void user_pwm_led_on(u16 pwm_ch,u16 h_duty);
extern void user_pwm_open(u16 pwm_ch);
extern void user_pwm_close(u16 pwm_ch);
void led_flash()
{
    //echo_led_on(IO_PORTB_00);
    user_pwm_open(0);
    //user_pwm_led_on(0,5000);
    os_time_dly(20);
    //gpio_led_high_block(IO_PORTB_00);
    //user_pwm_led_on(0,0);
    user_pwm_close(0);
}
...
void led_flash_add()
{
    if(!led_time)
    {
        led_time = sys_timer_add(NULL,led_flash,500);
    }
}
...
#define   LED_IO_C      PORTC
#define   LED_IO_B      PORTB

#define   LED_188_IO_1  IO_PORTC_03//PORT_PIN_1
#define   LED_188_IO_2  IO_PORTC_04//PORT_PIN_1
#define   LED_188_IO_3  IO_PORTC_02//PORT_PIN_2
#define   LED_188_IO_4  IO_PORTB_03//PORT_PIN_3
#define   LED_188_IO_5  IO_PORTB_01//PORT_PIN_4




u8 nums[11] = {0x7E, 0x30, 0x6D, 0x79, 0x33, 0x5B, 0x5F, 0x70, 0x7F, 0x7B,0x00};
u8 num_hund[5] = {0x00, 0x01, 0x03,0x0C, 0x0F};

u8 hundreds = 3;	//百位，0不显示，1仅百分比，2百分比+供电，3-百分比+百位，4全部显示
u8 tens = 0; 		//十位，10-F，11-不显示
u8 unit = 0;		//个位，10-F，11-不显示

void all_led_gpio_off()
{
    #if 0
    echo_led_off(PORTC,PORT_PIN_3,IO_PORTC_03);
    echo_led_off(PORTC,PORT_PIN_4,IO_PORTC_04);
    echo_led_off(PORTC,PORT_PIN_2,IO_PORTC_02);
    echo_led_off(PORTB,PORT_PIN_3,IO_PORTB_03);
    echo_led_off(PORTB,PORT_PIN_1,IO_PORTB_01);
    #else
    gpio_led_high_block(PORTC,PORT_PIN_3,IO_PORTC_03);
    gpio_led_high_block(PORTC,PORT_PIN_4,IO_PORTC_04);
    gpio_led_high_block(PORTC,PORT_PIN_2,IO_PORTC_02);
    gpio_led_high_block(PORTB,PORT_PIN_3,IO_PORTB_03);
    gpio_led_high_block(PORTB,PORT_PIN_1,IO_PORTB_01);
    #endif
}
...
{
    echo_led_off(PORTC,PORT_PIN_3,IO_PORTC_03);				//拉低Pin1
    if(nums[unit] & BIT(5))	//B3
    	echo_led_on(PORTC,PORT_PIN_4,IO_PORTC_04);
    if(nums[unit] & BIT(3))	//D3
		echo_led_on(PORTC,PORT_PIN_2,IO_PORTC_02);
	if(nums[unit] & BIT(1))	//F3
		echo_led_on(PORTB,PORT_PIN_3,IO_PORTB_03);
	if(nums[unit] & BIT(0))	//G3
		echo_led_on(PORTB,PORT_PIN_1,IO_PORTB_01);
}
...
{
    echo_led_off(PORTC,PORT_PIN_4,IO_PORTC_04);
    if(nums[unit] & BIT(6))	//A3
    	echo_led_on(PORTC,PORT_PIN_3,IO_PORTC_03);
	if(nums[tens] & BIT(5))	//B2
		echo_led_on(PORTC,PORT_PIN_2,IO_PORTC_02);
	if(nums[tens] & BIT(3))	//D2
		echo_led_on(PORTB,PORT_PIN_3,IO_PORTB_03);
	if(nums[tens] & BIT(2))	//E2
		echo_led_on(PORTB,PORT_PIN_1,IO_PORTB_01);
}
...
{
    echo_led_off(PORTC,PORT_PIN_2,IO_PORTC_02);
    if(nums[unit] & BIT(4))	//C3
    	echo_led_on(PORTC,PORT_PIN_3,IO_PORTC_03);
	if(nums[tens] & BIT(6))	//A2
		echo_led_on(PORTC,PORT_PIN_4,IO_PORTC_04);
	if(nums[tens] & BIT(4))	//C2
		echo_led_on(PORTB,PORT_PIN_3,IO_PORTB_03);
	if(nums[tens] & BIT(1))	//F2
		echo_led_on(PORTB,PORT_PIN_1,IO_PORTB_01);
...
{
    echo_led_off(PORTB,PORT_PIN_3,IO_PORTB_03);
    if(nums[unit] & BIT(2))			//E3
    	echo_led_on(PORTC,PORT_PIN_3,IO_PORTC_03);
	if(num_hund[hundreds] & BIT(2))	//C1
		echo_led_on(PORTC,PORT_PIN_4,IO_PORTC_04);
	if(num_hund[hundreds] & BIT(3))	//B1
		echo_led_on(PORTC,PORT_PIN_2,IO_PORTC_02);
	if(nums[tens] & BIT(0))			//G2
		echo_led_on(PORTB,PORT_PIN_1,IO_PORTB_01);
...
static int led_seg = 0;


void led_water_flash()
{
    my_delay_us(1);
    //if(led7_flash)
    {
...
static u16 led_water_time = 0;
void led_water_flash_add()
{
    if(!led_water_time)
    {
        led_water_time = usr_timer_add(NULL,led_water_flash,1,1);
    }
}

void led_water_flash_del()
{
    if(led_water_time)
    {
        usr_timer_del(led_water_time);
        led_water_time = 0;
    }
}


static u16 led_charge_water_time = 0;
void led_charge_water_flash_add()
{
    //if(!led_water_time)
    {
        led_charge_water_time = sys_timer_add(NULL,led_water_flash,1);
    }
}

void led_charge_water_flash_del()
{
    if(led_charge_water_time)
    {
        sys_timer_del(led_charge_water_time);
        led_charge_water_time = 0;
    }
}

static u16 led7_flash = 1;
static u16 led7_charge_flash_time = 0;

void led7_charge_flash()
{
    if(led7_flash)
    {
        led_water_flash_add();
...
void led7_charge_flash_add()
{
    if(!led7_charge_flash_time)
        led7_charge_flash_time = sys_timer_add(NULL,led7_charge_flash,900);
}

void led7_charge_flash_del()
{
    if(led7_charge_flash_time)
    {
        sys_timer_del(led7_charge_flash_time);
        led7_charge_flash_time = 0;
    }
}
